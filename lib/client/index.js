...
// testing, speed up careportal on Android keyboard show/hide
...
if (saveWindowWidth == true){
   ...
   if(window.innerWidth){
     ...
     savedWindowWidth = document.body.clientWidth;
   }
...
// clears the current user brush and resets to the current real time data
  ...
  function brushed ( ) {
    ...
    function updatePlugins (time) {
      ...
      // why can't we get this '---' updated immediately on page load, I hate waiting for 3-12 seconds
      // for all the javascript "wiring" and data loading events etc.
  ...
  function refreshMobileSafe() {
         // this function tries to ignore resize events fired due to Android's constant
         // resizing of the viewscreen due to its onscreen keyboard pushing the window size.
        ...
        // here we assume user has resized the chrome window, likely by rotating
        // the phone/table and they'd really like to see it redrawn to fit.
        if(savedWindowWidth != windowWidth) {
               // do something
                console.info('Window resized width = ' + windowWidth + 'px (Android Keyboard issue), updating - ' + new Date());
                refreshChart(true);
    ...
    // alternative to tracking all the window widths and calling refreshChart on resize,
    // just monitor for portrait/landscape rotation of phone.
  ...
  // Android onscreen keyboard: attempted workaround to the extremely slow browser
  // display when changing input form fields, in the background the hidden chart
  // was trying to repaint at the new short height due to the Android keyboard bug/feature.
  // window.onresize = refreshChart;
  window.onresize = refreshMobileSafe;


  // finally? load clock instead of '---'
  updateClock();
  updateTimeAgoSoon();
    
  // iOS safari might need refreshChart called on startup since I changed the onresize listener -db
  // finally? load BSG instead of '---'
  // Cannot call this too soon or property 'brush' is undefined, so don't rotate your screen
  // while loading the page either.
  // refreshChart(true);
